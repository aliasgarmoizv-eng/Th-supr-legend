    ...
    try:
        ...
        async def start_bot():
            try:
                ...
                logger.error(
                    f"Bot is rate limited. Retrying in {error.retry_after} seconds"
                )
                ...
                logger.error(
                    f"Bot is rate limited. Retrying in {retry_after} seconds"
                )
                ...
                logger.error(
                    ...
                    f"{error.response.status} {error.response.reason}"
                )
        ...
        if BotConfig.DASHBOARD_ENABLED:
            ...
            logger.info(
                "\033[1;31mDashboard is disabled via config.\033[0m"
            )
